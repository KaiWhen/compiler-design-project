  ...
  for (i=0; i < SIZE; ++i){ 
	if (hash_table[i] != NULL){ 
		...
		while (l != NULL){ 
			...
			else if (l->st_type == STR_TYPE)          fprintf(of,"%-7s","real");
			else if (l->st_type == BOOL_TYPE)          fprintf(of,"%-7s","char");
			else if (l->st_type == FUNCTION_TYPE){
				fprintf(of,"%-7s %s","function returns ");
				if (l->inf_type == INT_TYPE) 		   fprintf(of,"%-7s","int");
				else if (l->inf_type  == STR_TYPE)    fprintf(of,"%-7s","real");
				else if (l->inf_type  == BOOL_TYPE)	   fprintf(of,"%-7s","char");
...
int get_type(char *name){ // get the type of an entry
	...
	/* if "simple" type */
	if(l->st_type == INT_TYPE || l->st_type == STR_TYPE || l->st_type == BOOL_TYPE){
		return l->st_type;
	}
	/* if array, pointer or function */
	else{
